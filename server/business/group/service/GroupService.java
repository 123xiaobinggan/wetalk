package com.wetalk.business.group.service;
...
public class GroupService {
    ...
    public HttpResult<Object> buildGroup(GroupBuildDTO dto) {
        ...
        try {
            ...
            List<NotifyPushUnit> notifyPushUnits = new ArrayList<>();
            for (int i = 0; i < userIds.length; i++) {
                notifyPushUnits.add(new NotifyPushUnit(userIds[i],
    ...
    public HttpResult<Object> joinGroup(GroupJoinDTO dto) {
        ...
        try {
            ...
            Conversation[] convs = conversationMapper.fetchConversationsByUserIdsAndSessionId(groupMemberUserIds,
                    groupId);
            List<NotifyPushUnit> notifyPushUnits1 = new ArrayList<>();
            List<NotifyPushUnit> notifyPushUnits2 = new ArrayList<>();
            for (Conversation c : convs) {
                // 推送新成员加入消息
                notifyPushUnits1.add(new NotifyPushUnit(c.getUserId(),
                        new WsResult(WsType.GROUP, GroupEvent.JOIN, 0,
                                "新成员加入", null,
                                new MembersJoinPush(group, newGroupMembers))));
                // 推送群聊系统消息
                notifyPushUnits2.add(
                        new NotifyPushUnit(c.getUserId(),
                                new WsResult(WsType.CHAT, ChatEvent.RECEIVE, 0, "新成员加入", null,
                                        new SendPush(messageVO, c))));

            }
            notifyPushProducer.pushNotification(new NotifyPush(notifyPushUnits1));
            notifyPushProducer.pushNotification(new NotifyPush(notifyPushUnits2));
        ...
        try {
            ...
            if (groupMembers.length == 0) {
                ...
                for (GroupMember member : groupMembers) {
                    notifyPushUnits.add(new NotifyPushUnit(member.getMemberUserId(), new WsResult(WsType.GROUP,
    ...
    public HttpResult<Object> setGroupMembersRole(GroupMembersRoleSetDTO dto) {
        ...
        try {
            ...
            for (Long memberUserId : groupMemberUserIds) {
                notifyPushUnits.add(new NotifyPushUnit(memberUserId,
                ...
                notifyPushUnits1.add(
                        new NotifyPushUnit(c.getUserId(),
                                new WsResult(WsType.CHAT, ChatEvent.RECEIVE, 0, "全员禁言", null,
                                        new SendPush(messageVO, c))));
                // 推送群聊更新事件
                notifyPushUnits2.add(new NotifyPushUnit(c.getUserId(),
        ...
        try {
            ...
            if (announcement != null) {
                ...
                for (Conversation c : convs) {
                    ...
                    notifyPushUnits.add(new NotifyPushUnit(c.getUserId(),
                            new WsResult(WsType.CHAT, ChatEvent.RECEIVE, 0, "新公告发布", null,
                                    new SendPush(messageVO, c))));
            ...
            // 推送给群成员
            for (Long memberUserId : groupMemberUserIds) {
                notifyPushUnits.add(new NotifyPushUnit(memberUserId, new WsResult(
    ...
    public HttpResult<Object> transferGroupOwner(GroupOwnerTransferDTO dto) {
        ...
        try {
            ...
            for (Long memberUserId : groupMemberUserIds) {
                // 群成员更新推送
                notifyPushUnits1.add(new NotifyPushUnit(memberUserId,
                        new WsResult(WsType.GROUP, GroupEvent.MEMBERS_UPDATE, 0, "群成员信息更新", null,
                                membersUpdatePush)));
                // 群聊更新推送
                notifyPushUnits2.add(
                        new NotifyPushUnit(memberUserId,
